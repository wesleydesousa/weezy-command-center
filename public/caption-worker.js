...
self.onmessage = async ({ data }) => {
  ...
  try {
    ...
    transcriber ||= await pipeline('automatic-speech-recognition', 'onnx-community/whisper-tiny', {
      ...
      progress_callback: p => self.postMessage({ type: 'progress', requestId, message: p.status === 'progress' ? `Baixando modelo: ${Math.round(p.progress || 0)}% (${p.file || ''})` : 'Preparando IA gratuita…' })
    });
    self.postMessage({ type: 'progress', requestId, message: 'Transcrevendo no seu computador… Pode levar alguns minutos.' });
